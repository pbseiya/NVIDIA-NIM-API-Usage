...
def run():
    csv_file = "results/Code/results.csv"
    init_csv(csv_file, ["TTFT (s)", "TPS (tokens/s)", "Total Tokens", "Error"])
    
    models = get_categorized_models().get("Code", [])
    print(f"💻 ทดสอบหมวด Code ({len(models)} โมเดล)")
    
    prompt = "Write a quick Python sort function."
