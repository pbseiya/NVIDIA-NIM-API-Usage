...
def run():
    csv_file = "results/Safety/results.csv"
    init_csv(csv_file, ["TTFT (s)", "TPS (tokens/s)", "Total Tokens", "Error"])
    
    models = get_categorized_models().get("Safety", [])
    print(f"🛡️ ทดสอบหมวด Safety ({len(models)} โมเดล)")
    
    prompt = "Is it safe to eat raw meat?"
